...
- `0013_gpu_execution_batch_task_contracts.sql` is a forward-only companion to migration `0012`. It adds Worker-identity, batch-member, claimed-attempt, GPU/model and monotonic lifecycle checks for batch heartbeat, complete and fail RPCs. On 2026-07-21 the linked remote database applied migrations `0001` through `0013`; a synthetic three-member batch proved ordered batch claim, scoped heartbeat, idempotent completion and normal-user denial. All synthetic records were deleted.
...
- RTX 4090 remains exact model / 24GB VRAM / 32GB RAM hard / 96GB preferred / 200GB disk hard. RTX 5090 remains exact model / 31-32GB display tolerance only / 80GB RAM hard / 128GB preferred / 250GB disk hard. The scheduler helper uses the larger of the hard disk floor and 120% of planned sync size.
...
- Added shared GPU profile logic for `rtx4090` and `rtx5090`. RTX 4090 requires exact `NVIDIA GeForce RTX 4090`, at least 24GB VRAM, 32GB RAM hard minimum, 96GB preferred RAM, 200GB disk hard minimum, 250GB preferred disk, and aggressive offload. RTX 5090 requires exact `NVIDIA GeForce RTX 5090`, the existing 31/32GB VRAM tolerance, 80GB RAM hard minimum, 128GB preferred RAM, 250GB disk hard minimum, 300GB preferred disk, and balanced offload.
