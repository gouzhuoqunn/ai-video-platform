...
- The restricted Clore diagnostic agent can run from a deterministic bundle containing only the agent plus controller and workflow source. Its controller stage verifies the bundle manifest and does not clone the private project repository. The existing R2 presigned-GET helper currently caps expiry at 7,200 seconds; a requested three-hour diagnostic bootstrap must stop before rental unless that capability is deliberately extended.
...
- `scripts/clore/run-image-e2e.ts` uses this local claim/finalize contract. It must not claim a task until the remote runtime and all five model files have succeeded; no Supabase image queue, Storage path, or cloud credential is part of this image E2E path.
