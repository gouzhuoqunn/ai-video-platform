...
- Video archiving now resolves the repository-bundled ffmpeg binary, reports spawn errors, atomically preserves `source.webm` before conversion, removes it only after successful MP4/thumbnail creation, and has a local conversion/thumbnail regression. No second GPU order was created for this post-cleanup fix.
