...
The isolated local environment is Python 3.10.11 with `torch 2.6.0+cpu` and `torchaudio 2.6.0+cpu`; CUDA is unavailable by design. PyPI's official `pyopenjtalk==0.4.1` release exposes only the source distribution (SHA-256 `d5ada46f7fc2b52c1c79c273eb9668ff6ad7ab276a8db9d8be119ef93440f0dc`) for this target. The reproducible GitHub-hosted Windows build workflow is present, but an artifact has not yet been retrieved and verified locally. The Runtime and model package therefore remain `blocked`: no pretrained model files were downloaded and no GPT-SoVITS synthesis was claimed.
