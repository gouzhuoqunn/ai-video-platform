...
const candidates = findBootstrapImageCandidates([
  ...
  { id: 20002, gpu_name: "NVIDIA RTX A5000", gpu_memory_gb: 16, ram_gb: 32, disk_gb: 120, supports_docker: true, supports_ssh: true, rentable: true, type: "on-demand", price_usd_per_hour: 0.5, rating: 4.9, reliability: 0.99 },
...
assert.equal(candidates[0].runtimeGpuProfile, "bootstrap_image_gpu");
assert.equal(candidates[1].serverId, "20001");
assert.ok(candidates.every((candidate) => candidate.projectedFirstImageCostUsd !== null && candidate.projectedFirstImageCostUsd <= 4.5));
