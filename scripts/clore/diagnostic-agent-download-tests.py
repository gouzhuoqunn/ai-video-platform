...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp:
        root=Path(temp); agent.ROOT=root; agent.STATE_FILE=root/"state.json"; agent.STATE={"alive":True,"stages":{},"models":{}}; data=b"abcdefghij"; item=entry(agent,root,data); calls=[]; original=agent.open_model_download
        def fresh(request, **_): calls.append(request); return Response(data,200,{"Content-Length":str(len(data))})
        agent.open_model_download=fresh
        ...
        agent.open_model_download=resume
        ...
        agent.open_model_download=lambda *_args,**_kwargs: Response(data[4:],206,{"Content-Range":"bytes 3-9/10","Content-Length":"6"})
        try:
        ...
        agent.open_model_download=short
        try:
            try: agent.stream_model(item)
