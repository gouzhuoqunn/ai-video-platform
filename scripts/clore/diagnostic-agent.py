...
def test_stage_delay():
    try: return min(5.0, max(0.0, float(os.environ.get("DIAG_TEST_STAGE_DELAY_SECONDS", "0"))))
...
def complete(name, data):
    with STATE_LOCK:
        record = STATE["stages"][name]; record.update({"status": "succeeded", "completed_at": now(), "exit_code": 0, "data": data}); STATE["current_stage"] = "idle"; save_locked()

def failed(name, error, data=None):
    with STATE_LOCK:
        record = STATE["stages"].setdefault(name, {}); record.update({"status": "failed", "completed_at": now(), "first_exact_failure": clean(error), "data": data or record.get("data", {})}); STATE["last_error"] = clean(error); STATE["current_stage"] = "idle"; save_locked()
...
def run_stage(route):
    ...
    if not STAGE_GATE.acquire(blocking=False): return False
    def worker():
        try:
            begin(name); time.sleep(test_stage_delay()); complete(name, action())
        except Exception as error: failed(name, error)
        finally: STAGE_GATE.release()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        with STATE_LOCK:
            if route == "/healthz": value = {"alive": True, "current_stage": STATE["current_stage"], "last_error": STATE["last_error"]}
            elif route == "/status": value = json.loads(json.dumps(STATE))
            elif route == "/logs": value = {name: record.get("log_tail", []) for name, record in STATE["stages"].items()}
            else: value = None
        if route == "/healthz": self.send_json(200, value); return
        if route == "/status": self.send_json(200, value); return
        if route == "/logs": self.send_json(200, value); return
...
def main():
    parser = argparse.ArgumentParser(); parser.add_argument("--token-sha256", required=True); parser.add_argument("--port", type=int, default=8080); args = parser.parse_args()
    ROOT.mkdir(parents=True, exist_ok=True); LOG_DIR.mkdir(parents=True, exist_ok=True)
    with STATE_LOCK: STATE["started_at"] = now(); save_locked()
