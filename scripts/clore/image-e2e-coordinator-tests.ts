...
async function main() {
  ...
  await assert.rejects(() => runImageE2e({ taskId: task.id, immutableCommit: "a".repeat(40), tokenFile: ".secrets/token", tokenSha256: "d".repeat(64), resume: false }, explicitInferenceFailure.deps), /inference_stage_failed:controller_exact_failure/);
