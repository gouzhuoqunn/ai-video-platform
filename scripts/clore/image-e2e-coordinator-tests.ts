...
async function main() {
  ...
  const resumedModels = fake({ modelIncomplete: "once" }); await runImageE2e({ taskId: task.id, immutableCommit: "a".repeat(40), tokenFile: ".secrets/token", tokenSha256: "d".repeat(64), resume: false }, resumedModels.deps);
  assert.equal(resumedModels.events.filter((value) => value === "stage:models").length, 2); assert.equal(resumedModels.inferenceCalls, 1);
  const incompleteExhausted = fake({ modelIncomplete: "twice" }); await assert.rejects(() => runImageE2e({ taskId: task.id, immutableCommit: "a".repeat(40), tokenFile: ".secrets/token", tokenSha256: "d".repeat(64), resume: false }, incompleteExhausted.deps), /model_download_incomplete_after_refresh/); assert.equal(incompleteExhausted.events.filter((value) => value === "stage:models").length, 2); assert.equal(incompleteExhausted.inferenceCalls, 0);
