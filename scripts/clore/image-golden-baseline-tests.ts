...
// The golden record preserves the agent hash that produced the historic image.
// Stage correlation intentionally changes the current Agent without changing that
// artifact evidence; current Agent integrity is checked by diagnostic-agent-tests.
assert.match(baseline.runtime.agentSha256, /^[a-f0-9]{64}$/);
...
console.log(JSON.stringify({ ok: true, golden_runtime_evidence_immutable: true, golden_png_evidence_immutable: true, golden_completed_task_unchanged: true, current_agent_intentionally_revised: true, credentials_absent: true }));
