...
const fixedConfig = () => ({ ...loadCloreConfig(), targetGpu: "NVIDIA GeForce RTX 4090" as const, minGpuVramGb: 24, maxGpuPricePerHour: IMAGE_SESSION_LIMITS.maxHourlyUsd, orderType: "on-demand" as const });
...
function exactTaskIds(taskIds: readonly string[]) { if (!taskIds.length || new Set(taskIds).size !== taskIds.length) throw new Error("image_session_exact_task_ids_required"); return taskIds.map((id) => resolveExactEligibleImageTask(id)); }
...
export async function createLiveSessionOrder(input: { sessionId: string; taskIds: string[]; immutable: ImmutableRuntime; execute: boolean }): Promise<LiveSessionOrder> {
  ...
  const candidates = market.map((raw) => normalizeCloreServer(raw, config)).filter((item) => item.gpuNormalizedName === "NVIDIA GeForce RTX 4090" && item.orderType === "on-demand" && item.hostOnline !== false && item.priceUsdPerHour !== null && item.priceUsdPerHour <= IMAGE_SESSION_LIMITS.maxHourlyUsd && item.priceOriginalAmount !== null && item.priceOriginalUnit === "day");
  const selected = candidates.find((item) => item.serverId === "98682") ?? null;
  if (!selected?.priceUsdPerHour || !selected.priceOriginalAmount) throw new Error("preferred_rtx4090_98682_unavailable");
  const wallet = await readWalletSummary(config, { forceRefresh: true });
  const plan = planImageSession(tasks, { activeOrderCount: 0, selectedHourlyUsd: selected.priceUsdPerHour, walletBalanceUsd: wallet.availableUsdBalance });
...
export async function submitTaskInference(order: LiveSessionOrder, task: EligibleImageTask, receipt?: { submitting: () => void; accepted: (value: AcceptedStage) => void }): Promise<{ accepted: AcceptedStage; terminal: StageTerminal; artifact: LocalArtifactReference; controllerJobId: string | null; controllerPromptId: string | null }> {
  ...
  try {
    ...
    const terminal = await pollInferenceStage(order.endpoint, order.token, accepted.stageRunId, { reconcileExactOrder: async () => Boolean((await readLiveOrdersSummary(fixedConfig(), { forceRefresh: true })).find((item) => item.orderId === order.orderId)?.active) });
