...
const requiredChecks: CheckDefinition[] = [
  ...
  { name: "live_cheapest_candidate_ranking", files: ["scripts/clore/live-market-selection-tests.ts"] },
  ...
  { name: "golden_rtx4090_deployment_profile", files: ["scripts/rtx4090-golden-deployment-profile-tests.ts", "scripts/image-executor-focused-tests.ts"] },
  { name: "pinned_agent_stage_acceptance_v2", files: ["scripts/clore/pinned-agent-http-contract-tests.ts", "scripts/clore/agent-stage-acceptance-contract-tests.ts"] },
...
try {
  for (const [index, file] of testFiles.entries()) {
    ...
    const result = spawnSync(process.execPath, ["--import", "tsx", file], {
      cwd: process.cwd(),
      env: { ...process.env, AI_IMAGE_TASK_STORE_PATH: isolatedTaskStore },
      encoding: "utf8",
      windowsHide: true,
