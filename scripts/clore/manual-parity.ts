...
export type ManualParitySessionState = {
  ...
  passwordConfiguredInPayload: boolean;
...
export function readManualParityState(filePath = MANUAL_PARITY_STATE_PATH) {
  ...
  return {
    ...state,
    passwordConfiguredInPayload: state.passwordConfiguredInPayload === true,
    passwordAttempted: state.passwordAttempted === true,
  };
}
...
export function createManualParityState(serverId: string, filePath = MANUAL_PARITY_STATE_PATH) {
  const now = new Date();
  return writeManualParityState({ schemaVersion: 1, serverId, orderId: null, createdAt: now.toISOString(), expiresAt: new Date(now.getTime() + 180 * 60_000).toISOString(), sshPassword: generateStrongSshPassword(), sshHost: null, sshPort: null, passwordAuthSucceeded: false, keyInstalled: false, keyAuthSucceeded: false, passwordConfiguredInPayload: false, passwordAttempted: false }, filePath);
}
...
export function passwordFallbackAvailableForOrder(state: ManualParitySessionState | null, orderId: string) {
  return Boolean(
    state &&
    state.orderId === orderId &&
    state.passwordConfiguredInPayload &&
    !state.passwordAttempted,
  );
}

export function consumePasswordFallbackAttempt(orderId: string, filePath = MANUAL_PARITY_STATE_PATH) {
  const state = readManualParityState(filePath);
  if (!passwordFallbackAvailableForOrder(state, orderId) || !state) {
    throw new Error("order_password_fallback_not_configured");
  }
  return writeManualParityState({ ...state, passwordAttempted: true }, filePath);
}

export function repairAuthorizedKeysText(existing: string, canonicalPublicKey: string) {
  const lines = existing.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
  if (!lines.includes(canonicalPublicKey)) lines.push(canonicalPublicKey);
  return `${lines.join("\n")}\n`;
}
