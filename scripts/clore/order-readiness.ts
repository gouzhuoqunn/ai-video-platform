...
import { buildSshArgs, getPublicKeyPath, type SshTarget } from "./ssh-client";
...
function passwordSsh(target: SshTarget, command: string, knownHostsPath: string) {
  ...
  const args = ["-o", `UserKnownHostsFile=${knownHostsPath}`, "-o", "StrictHostKeyChecking=accept-new", "-o", "PreferredAuthentications=password", "-o", "PubkeyAuthentication=no", "-o", "NumberOfPasswordPrompts=1", "-o", "ConnectTimeout=15", "-T", "-p", String(target.port), `${target.user}@${target.host}`, command];
      ...
      if (latest.deploymentReady && latest.ssh) {
        ...
        if (tcpReached) {
          ...
          if (parity && parity.serverId === latest.serverId && (!parity.orderId || parity.orderId === orderId)) {
            const password = passwordSsh(sshTarget, "true", knownHostsPath); passwordAuthSucceeded = password.ok;
            if (password.ok) {
              const installed = passwordSsh(sshTarget, projectKeyInstallCommand(config.sshPublicKeyPath), knownHostsPath); keyInstalled = installed.ok;
              if (installed.ok) { const key = keySsh(sshTarget, "true", knownHostsPath); keyAuthSucceeded = key.ok; }
            } else {
              const existingKey = keySsh(sshTarget, "true", knownHostsPath);
              if (existingKey.ok) {
                const installed = keySsh(sshTarget, projectKeyInstallCommand(config.sshPublicKeyPath), knownHostsPath); keyInstalled = installed.ok;
                if (installed.ok) { const key = keySsh(sshTarget, "true", knownHostsPath); keyAuthSucceeded = key.ok; }
              }
            }
            ...
            authFailed = !readiness.ready;
            tcpReached = readiness.classification !== "ssh_tcp_not_ready";
            if (readiness.target) latest = latest ? { ...latest, ssh: readiness.target } : latest;
          }
          issue = readinessIssue(latest, { tcpReached, authSucceeded: keyAuthSucceeded, authFailed });
          if (issue === "ssh_ready") {
            if (parity) updateManualParityState({ orderId, sshHost: sshTarget.host, sshPort: sshTarget.port, passwordAuthSucceeded, keyInstalled, keyAuthSucceeded });
            ...
            writeFileSync(targetPath, `${JSON.stringify({ order_id: orderId, ...sshTarget, username: sshTarget.user, gpuProfile: active?.gpu_profile ?? "rtx4090", runtimeDigest: FIXED_RUNTIME_DIGEST, knownHostsPath, ssh_auth: { passwordAuthSucceeded, keyInstalled, keyAuthSucceeded, classification: "ssh_ready", attempts: keyAttempts || 1, readinessDurationMs: keyReadinessDurationMs }, updated_at: new Date().toISOString() }, null, 2)}\n`, { encoding: "utf8", mode: 0o600 });
          ...
          if (!parity && sshClassification) break;
        }
      }
    }
    await sleep(10_000);
