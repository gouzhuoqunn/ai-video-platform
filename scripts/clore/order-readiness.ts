...
import { ensureManualParityAskpass, MANUAL_PARITY_NODE_PRELOAD_PATH, MANUAL_PARITY_STATE_PATH, readManualParityState, updateManualParityState } from "./manual-parity";
...
function installProjectKey(target: SshTarget, configuredPublicKeyPath?: string) {
  ...
  return passwordSsh(target, `set -e; umask 077; mkdir -p /root/.ssh; touch /root/.ssh/authorized_keys; grep -qxF ${quoted} /root/.ssh/authorized_keys || printf '%s\\n' ${quoted} >> /root/.ssh/authorized_keys; chmod 700 /root/.ssh; chmod 600 /root/.ssh/authorized_keys`);
}
      ...
      if (latest.deploymentReady && latest.ssh) {
        ...
        if (tcpReached) {
          ...
          if (parity && parity.serverId === latest.serverId && (!parity.orderId || parity.orderId === orderId)) {
            const password = passwordSsh(latest.ssh, "true"); passwordAuthSucceeded = password.ok; authFailed ||= password.authFailed;
            if (password.ok) {
              const installed = installProjectKey(latest.ssh, config.sshPublicKeyPath); keyInstalled = installed.ok; authFailed ||= installed.authFailed;
              if (installed.ok) { const key = keySsh(latest.ssh, "true"); keyAuthSucceeded = key.ok; authFailed ||= key.authFailed; }
            }
          } else {
            const key = keySsh(latest.ssh, "true"); keyAuthSucceeded = key.ok; authFailed ||= key.authFailed;
          }
          issue = readinessIssue(latest, { tcpReached, authSucceeded: keyAuthSucceeded, authFailed });
          if (issue === "ssh_ready") {
            ...
            writeFileSync(targetPath, `${JSON.stringify({ order_id: orderId, ...latest.ssh, username: latest.ssh.user, gpuProfile: active?.gpu_profile ?? "rtx4090", runtimeDigest: FIXED_RUNTIME_DIGEST, updated_at: new Date().toISOString() }, null, 2)}\n`, { encoding: "utf8", mode: 0o600 });
