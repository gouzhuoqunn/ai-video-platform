...
type FreshReceipt={schema:1;runId:string;taskId:string;orderId:string|null;endpoint:string|null;currentStep:string;inferenceState:"not_started"|"submitting"|"accepted"|"succeeded"|"failed";inferenceSubmitted:boolean;inferenceSucceeded:boolean;inferenceSubmittingAt:string|null;inferenceAcceptedAt:string|null;acceptedHttpStatus:202|null;inferenceCompletedAt:string|null;remoteArtifactAvailable:boolean;controllerPromptId:string|null;remoteArtifact:{byteSize:number;sha256:string;width:number;height:number;generationDurationSeconds:number}|null;artifactDownloaded:boolean;localArtifactPublished:boolean;taskFinalized:boolean;uiVerified:boolean;orderCancelled:boolean;timestamps:Record<string,string>;firstError:string|null};
function receiptRead(){try{return JSON.parse(readFileSync(RECEIPT_PATH,"utf8")) as FreshReceipt}catch{return null}}
function receiptWrite(value:FreshReceipt){mkdirSync(path.dirname(RECEIPT_PATH),{recursive:true});const tmp=`${RECEIPT_PATH}.${process.pid}.tmp`;const descriptor=openSync(tmp,"w",0o600);try{writeSync(descriptor,`${JSON.stringify(value,null,2)}\n`);fsyncSync(descriptor)}finally{closeSync(descriptor)}renameSync(tmp,RECEIPT_PATH)}
...
async function main() {
  ...
  if (process.argv.includes("--execute-fresh")) {
    ...
    const task=readImageTask(taskId);const old=receiptRead();if(freshRunRequiresManualRecovery(old,taskId,task?.status))throw new Error("previous_inference_state_requires_manual_recovery");
    const receipt:FreshReceipt={schema:1,runId:randomBytes(12).toString("hex"),taskId,orderId:null,endpoint:null,currentStep:"prerental",inferenceState:"not_started",inferenceSubmitted:false,inferenceSucceeded:false,inferenceSubmittingAt:null,inferenceAcceptedAt:null,acceptedHttpStatus:null,inferenceCompletedAt:null,remoteArtifactAvailable:false,controllerPromptId:null,remoteArtifact:null,artifactDownloaded:false,localArtifactPublished:false,taskFinalized:false,uiVerified:false,orderCancelled:false,timestamps:{prerental:new Date().toISOString()},firstError:null};receiptWrite(receipt);ACTIVE_RECEIPT=receipt;
