...
assert.match(launchComfy, /async def load_profile_builtin_extra_nodes/);
assert.match(launchComfy, /async def init_profile_builtin_extra_nodes/);
assert.match(launchComfy, /await load_profile_builtin_extra_nodes/);
assert.match(launchComfy, /await nodes_module\.load_custom_node/);
assert.match(launchComfy, /AWAITING_PROFILE_EXTRA/);
assert.match(launchComfy, /AWAITED_PROFILE_EXTRA/);
assert.doesNotMatch(launchComfy, /asyncio\.gather|create_task|asyncio\.run|run_until_complete/);
...
assert.doesNotMatch(workflow, /D3 Locate builtin extra crash source in original order/);
assert.match(workflow, /dependency_audit_has_risk=false/);
assert.match(workflow, /production_minimal_async_loader_cpu_boot_proven/);
