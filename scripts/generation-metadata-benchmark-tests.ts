...
import { FIRST_ROUND_CAPACITY_PLAN, PRODUCTION_PROFILE_PATHS, withDiskReserve } from "../src/lib/generation/r2-cache-plan";
...
function main() {
  ...
  for (const candidate of MODEL_CANDIDATES) {
    ...
    if (candidate.benchmark_round === "second") assert.notEqual(candidate.status, "production", "second-round candidates cannot be production");
    if (candidate.status === "metadata_verified") assert.match(candidate.revision, /^[a-f0-9]{40}$/, "verified candidates need immutable revisions");
  }
  for (const audit of FIRST_ROUND_METADATA_AUDITS) {
    ...
    if (audit.access === "gated") {
      ...
      assert.equal(audit.revision, null, "gated metadata must not invent a revision");
    } else {
