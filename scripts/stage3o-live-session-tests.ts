...
assert.ok(firstImageSource.includes("probeOptionalRuntimeTunnel") && firstImageSource.includes("runtime_websocket_optional") && firstImageSource.includes("runRemoteComfyWorkflow"), "SSH tunnel evidence must be optional while generation runs on the remote host");
const runnerSource = readFileSync("scripts/comfy-remote-runner.py", "utf8"); assert.ok(runnerSource.includes('BASE_URL = "http://127.0.0.1:8188"')); assert.ok(runnerSource.includes('require_json("/history/"')); assert.ok(runnerSource.includes('require_json("/queue")')); assert.ok(runnerSource.includes('"websocket_optional": True'));
