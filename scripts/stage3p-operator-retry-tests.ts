...
assert.equal(validateOperatorRetryOverride(buildOperatorRetryOverride(new Date(), 180, { maxAttempts: 2, maxFailedSpendUsd: 0.4, maxSessionSpendUsd: 2.5 })).valid, true);
...
assert.ok(liveSource.includes("setCloreDeploymentHold(true, \"stage3r_finally\")"));
assert.ok(liveSource.includes("clore:watchdog:remote:disarm"));
assert.ok(liveSource.includes("MAX_HOST_ATTEMPTS = 2"));
...
console.log("Stage 3R operator override validation, atomic consumption, two-attempt budget, independence, and cleanup guards passed.");
