...
const gpuState = readFileSync(path.join(process.cwd(), "src", "lib", "generation", "gpu-execution-state.ts"), "utf8");
...
assert.match(gpuState, /开始任务并租用显卡/);
assert.match(studio, /尚未选择执行队列/);
assert.match(studio, /GPU空闲，将在 \{idleCancelSeconds\} 秒后自动退租/);
