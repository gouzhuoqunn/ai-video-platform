...
type PoolSummary = {
  ...
  manualBatch?: {
    ...
    startIntent?: { status: string; selectedServerId?: string | null; selectedEffectiveHourlyUsd?: number | null; selectedHost?: Record<string, unknown> | null; lastError: string | null; orderPayload?: { image: string; ports: string[]; commandPresent: boolean; autosshEntrypoint: boolean | null; serverId: string | null; orderId: string | null; requiredPrice: number | null; deploymentState: string | null; startedAt: string | null; sshHost: string | null; sshPort: number | null; latestSshAuthError: string | null; createResponseStatus: string | null; updatedAt: string } | null } | null;
      ...
      <div className="studio-shell grid w-full gap-3 px-2 py-3">
        ...
        <aside className="studio-sidebar space-y-3" data-testid="permanent-gpu-sidebar">
          ...
          {activeManualBatch ? <section className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm" data-testid="gpu-runner-process-panel">
            {activeManualBatch.startIntent?.orderPayload ? <div className="mb-2 rounded border border-stone-200 bg-[#faf8f4] p-2 text-xs text-stone-800" data-testid="clore-order-payload-summary"><p className="font-semibold">真实订单载荷（已脱敏）</p><p className="mt-1 break-all">镜像：{activeManualBatch.startIntent.orderPayload.image}</p><p>端口：{activeManualBatch.startIntent.orderPayload.ports.join(", ")} · 命令：{activeManualBatch.startIntent.orderPayload.commandPresent ? "有" : "无"} · autossh：{activeManualBatch.startIntent.orderPayload.autosshEntrypoint === null ? "未发送" : String(activeManualBatch.startIntent.orderPayload.autosshEntrypoint)}</p><p>服务器：{activeManualBatch.startIntent.orderPayload.serverId ?? "—"} · 订单：{activeManualBatch.startIntent.orderPayload.orderId ?? "等待创建"} · required_price：{activeManualBatch.startIntent.orderPayload.requiredPrice ?? "—"}</p><p>部署：{activeManualBatch.startIntent.orderPayload.deploymentState ?? "等待 Clore 状态"} · 开始：{activeManualBatch.startIntent.orderPayload.startedAt ? new Date(activeManualBatch.startIntent.orderPayload.startedAt).toLocaleString() : "—"}</p><p>SSH：{activeManualBatch.startIntent.orderPayload.sshHost ? `${activeManualBatch.startIntent.orderPayload.sshHost}:${activeManualBatch.startIntent.orderPayload.sshPort ?? "—"}` : "尚未发布"}</p>{activeManualBatch.startIntent.orderPayload.latestSshAuthError ? <p className="mt-1 break-words text-rose-800">最新 SSH 认证错误：{activeManualBatch.startIntent.orderPayload.latestSshAuthError}</p> : null}</div> : null}
