...
export function projectRunnerStatus(runner: RunnerLike, state: { pidAlive: boolean; activeOrder: boolean; createLock: boolean }): RunnerStatusProjection {
  ...
  const historical = ["idle", "failed", "completed"].includes(runner.state) && !state.pidAlive && !state.activeOrder && !state.createLock && !runner.host?.orderId;
  if (historical) {
    const displayMessage = rentedCandidateCode6(runner.error.message) ? HISTORICAL_RENTED_CANDIDATE_MESSAGE : HISTORICAL_RUNNER_GENERIC_MESSAGE;
    return {
